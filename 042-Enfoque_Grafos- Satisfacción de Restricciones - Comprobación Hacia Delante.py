...
class ProblemaCSP:
    ...
    def resolver(self):
        if self.comprobacion_adelante():
            return self.solucion
        else:
            return None

    def comprobacion_adelante(self):
        ...
        for valor in self.dominios[variable]:
            if self.verificar_restricciones(variable, valor):
                ...
                dominios_guardados = self.guardar_dominios()  # Guardar los dominios actuales
                if self.comprobacion_adelante():  # Llamada recursiva
                    return True
                self.restaurar_dominios(dominios_guardados)  # Restaurar los dominios si no se encontró solución
                del self.solucion[variable]  # Retroceder si no se encontró solución
        return False
    ...
    def guardar_dominios(self):
        return {variable: list(self.dominios[variable]) for variable in self.variables}

    def restaurar_dominios(self, dominios_guardados):
